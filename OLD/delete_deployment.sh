...
# ========================
# 2.2 Delete HTTP proxies
# ========================
echo "Deleting HTTP proxies..."
for proxy in $(gcloud compute target-http-proxies list --format="value(name)" | grep "$PREFIX"); do
    echo "Deleting HTTP proxy: $proxy"
    gcloud compute target-http-proxies delete "$proxy" --quiet || true
    echo "HTTP proxy $proxy deleted"
done
echo "HTTP proxies deletion complete."
